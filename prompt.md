...
- **Missing Python**: Python is not a **primary** language for the role. (Note: Polyglot roles with Python ARE acceptable, e.g., as long as one the required programming languages is Python. We don't need to check relevance for other programming langugages for other than Python.)
...
- **Vague JD**: If the JD lists only high-level concepts (e.g., "distributed systems", "backend") without naming specific technologies, cap at 3.5 — no concrete skill overlap can be verified.
...
- **Specific**: Always parenthesize technologies, e.g., "Distributed Systems (Redis, etc. - if found)".
